...
import { PlusIcon, XIcon, TagIcon, BoxIcon, TruckIcon, DollarSignIcon } from "lucide-vue-next";
    ...
    <div class="container mx-auto py-10 px-4 sm:px-6 lg:px-8">
      ...
      <div class="bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <div class="flex border-b border-gray-200">
          <button v-for="tab in ['general', 'inventory', 'attributes', 'variations']" :key="tab"
            ...
            <component
              :is="tab === 'general' ? TagIcon : tab === 'inventory' ? BoxIcon : tab === 'variations' ? TruckIcon : TagIcon"
        ...
        <form @submit.prevent="submit" class="p-6">
          ...
          <div v-if="activeTab === 'general'">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <div class="relative rounded-md shadow-sm">
                  <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <DollarSignIcon class="h-5 w-5 text-gray-400" />
                  </div>
              ...
              <div>
                ...
                <input id="default_image" type="file" @change="(e) => (form.default_image = e.target.files[0])"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
                <p v-if="form.errors.default_image" class="mt-2 text-sm text-red-600">{{ form.errors.default_image }}
                </p>
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'attributes'">
            <div v-for="(attribute, index) in props.attributeValues" :key="index" class="mb-6">
              <label :for="'attribute-' + index" class="block text-sm font-medium text-gray-700 mb-2">
                {{ attribute.attribute_name }}
              </label>
              <div class="flex flex-wrap gap-5">
                <div v-for="value in attribute.values" :key="value.id" class="flex items-center">
                  <input :id="'attribute-value-' + value.id" type="checkbox" :value="value.id"
                    v-model="selectedAttributes[index].values" class="mr-2" />
                  <label :for="'attribute-value-' + value.id">{{ value.value }}</label>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'variations'">
            <div v-for="(variation, index) in form.variations" :key="index"
              ...
              <h3 class="text-lg font-medium text-gray-900 mb-4">Variation {{ index + 1 }}</h3>

              <!-- Attribute Select Inputs -->
              <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div v-for="(attribute, attrIndex) in variation.attributes" :key="attrIndex">
                  <label :for="'variation-attr-' + index + '-' + attrIndex"
                    class="block text-sm font-medium text-gray-700 mb-2">
                    {{ attribute.attribute_name }}
                  </label>
                  <select :id="'variation-attr-' + index + '-' + attrIndex" v-model="attribute.attribute_value_id"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    <option value="">Select {{ attribute.attribute_name }}</option>
                    <option
                      v-for="value in selectedAttributes.find(attr => attr.attribute_name === attribute.attribute_name)?.values"
                      :key="value" :value="value">
                      {{
                        props.attributeValues.find(attr => attr.attribute_name ===
                          attribute.attribute_name)?.values.find(val => val.id === value)?.value
                      }}
                    </option>
                  </select>
                </div>
              ...
              <!-- Price Input -->
              <div class="mt-4">
                <label :for="'variation-price-' + index" class="block text-sm font-medium text-gray-700 mb-2">
                  Price
                </label>
                <input type="number" :id="'variation-price-' + index" v-model="variation.price"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
              ...
              <!-- Stock Input -->
              <div class="mt-4">
                <label :for="'variation-stock-' + index" class="block text-sm font-medium text-gray-700 mb-2">
                  Stock
                </label>
                <input type="number" :id="'variation-stock-' + index" v-model="variation.stock"
                  class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
